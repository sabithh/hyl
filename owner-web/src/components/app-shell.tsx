...
export function AppShell({ title, subtitle, ownerName, gymName, children }: AppShellProps) {
      ...
      <div className="relative mx-auto flex max-w-[1500px] gap-4 p-4 lg:p-6">
        <aside className="sticky top-4 hidden h-[calc(100vh-2rem)] w-[260px] rounded-3xl border border-[color:var(--line)] bg-[color:var(--panel)] p-4 lg:block">
          <div className="mb-6 rounded-2xl bg-[color:var(--panel-strong)] p-4">
            <Image src="/hyl-logo.svg" alt="HYL" width={170} height={50} />
            <p className="mt-3 text-xs uppercase tracking-[0.22em] text-[color:var(--muted)]">HYL Owner Workspace</p>
            <h2 className="mt-1 text-lg font-bold text-[color:var(--mint)]">{gymName || "HYL"}</h2>
            <p className="mt-1 text-sm text-[color:var(--muted)]">{ownerName ? `Signed in: ${ownerName}` : "Control Center"}</p>
          </div>
        ...
        <main className="min-w-0 flex-1 rounded-3xl border border-[color:var(--line)] bg-[color:var(--panel)] p-4 sm:p-5 lg:p-7">
          ...
          <header className="mb-5 flex flex-wrap items-end justify-between gap-3 border-b border-[color:var(--line)] pb-4">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--muted)]">HYL Owner Workspace</p>
            ...
            <div className="flex items-center gap-2">
              <div className="rounded-xl bg-[color:var(--panel-strong)] px-3 py-2 text-sm text-[color:var(--muted)]">Live at {updatedTime}</div>
              <button
                type="button"
                onClick={handleLogout}
                disabled={loggingOut}
                className="rounded-xl border border-[color:var(--line)] bg-[color:var(--panel-strong)] px-3 py-2 text-sm font-semibold text-[color:var(--mint)] transition hover:bg-[color:var(--panel)] disabled:cursor-not-allowed disabled:opacity-70"
              >
